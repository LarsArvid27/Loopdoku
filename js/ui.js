class GameUI {
  ...
  onCellClick(index) {
    ...
    const availableCollaborators = collaboratorIds.filter(
      id => !usedCollaborators.includes(id) || id === cell.collaborator
    );
    ...
    if (nextCollaborator && !this.isValidPlacement(index)) {
  ...
  isValidPlacement(cellIndex) {
    ...
    for (let c = 0; c < 6; c++) {
      ...
      if (this.gameState.grid[idx].collaborator) return false;
    }
